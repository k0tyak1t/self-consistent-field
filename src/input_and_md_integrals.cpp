...
bool InputAndMDIntegrals::readGeom(char* filename)
{
    ...
    Atom atomTmp;
    double x, y, z;
    while (inp >> cur_str >> x >> y >> z) {
        atomTmp.q = get_nuclear_charge(cur_str);
        atomTmp.x = x;
        atomTmp.y = y;
        atomTmp.z = z;
        if (atomTmp.q == 0)
...
bool InputAndMDIntegrals::calc(standard_matrices& std_m)
{
    ...
    int ii, jj, kk, ll;
    ii = jj = 0;
    for (vector<vector<pair<int, SingleBasisFunction>>>::iterator i = basisFunctions.begin();
        i != basisFunctions.end(); i++) {
        jj = 0;
        for (vector<vector<pair<int, SingleBasisFunction>>>::iterator j = basisFunctions.begin();
    ...
    for (vector<vector<pair<int, SingleBasisFunction>>>::iterator i = basisFunctions.begin();
        ...
        for (vector<vector<pair<int, SingleBasisFunction>>>::iterator j = basisFunctions.begin();
            j != basisFunctions.end(); j++) {
            kk = ll = 0;
            for (vector<vector<pair<int, SingleBasisFunction>>>::iterator k = basisFunctions.begin();
                k != basisFunctions.end(); k++) {
                ll = 0;
                for (vector<vector<pair<int, SingleBasisFunction>>>::iterator l = basisFunctions.begin();
                    ...
                    for (vector<pair<int, SingleBasisFunction>>::iterator i1 = (*i).begin();
                        ...
                        for (vector<pair<int, SingleBasisFunction>>::iterator j1 = (*j).begin();
                            j1 != (*j).end(); j1++)
                            for (vector<pair<int, SingleBasisFunction>>::iterator k1 = (*k).begin();
                                k1 != (*k).end(); k1++)
                                for (vector<pair<int, SingleBasisFunction>>::iterator l1 = (*l).begin();
                                    l1 != (*l).end(); l1++) {
                                    double Vijkl_t = 0;
                                    SingleBasisFunction f1 = (*i1).second;
                                    SingleBasisFunction f2 = (*j1).second;
                                    ...
                                    for (vector<pair<double, double>>::iterator i2 = f1.ai_ci.begin();
                                        ...
                                        for (vector<pair<double, double>>::iterator j2 = f2.ai_ci.begin();
                                            j2 != f2.ai_ci.end(); j2++)
                                            for (vector<pair<double, double>>::iterator k2 = f3.ai_ci.begin();
                                                k2 != f3.ai_ci.end(); k2++)
                                                for (vector<pair<double, double>>::iterator l2 = f4.ai_ci.begin();
                                                    l2 != f4.ai_ci.end(); l2++)
                    ...
                    std_m.set_Vee(ii, jj, kk, ll, Vijkl);
                    ll++;
                }
                kk++;
            }
...
{
    int nBFs = 0;
    for (vector<vector<pair<int, SingleBasisFunction>>>::iterator it = basisFunctions.begin();
...
{
    int nBFs = 0;
    cout << "Проверка и установка C2v симметрии для линейных молекул, "
    ...
    for (vector<vector<pair<int, SingleBasisFunction>>>::iterator it = basisFunctions.begin();
        ...
        for (vector<pair<int, SingleBasisFunction>>::iterator jt = (*it).begin();
            ...
            if (((*jt).second.x != 0) || ((*jt).second.y != 0)) {
                cout << "Эта функция не лежит на оси z, симметрия не будет "
                        "установлена\n";
                isGood = false;
            }
...
bool InputAndMDIntegrals::makeBasis()
{
    ...
    for (vector<Atom>::iterator atom = atoms.begin(); atom != atoms.end();
        ...
        if (basis == basisLib.end()) {
            std::cerr << "Для атома с зарядом " << (*atom).q
                      << " в базисной библиотеке не был найден базис\n";
            return false;
        }

        for (vector<pair<int, vector<pair<double, double>>>>::iterator it1 = (*basis).second.begin();
