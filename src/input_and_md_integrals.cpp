...
bool InputAndMDIntegrals::readGeom(char* filename){
	...
	while(inp>>tmp_str>>x>>y>>z) {
		...
		atomTmp.y=y;
		atomTmp.z=z;
		if (atomTmp.q==0) return false;
		atoms.push_back(atomTmp);
	}
	inp.close();
	if (nAtoms!=int(atoms.size())) {
		cerr<<"Проверьте число атомов в файле \""<<filename<<"\"\n";
		return false;
	}
	return true;
}
	        ...
	        for (vector<pair<double, double> >::iterator i2=f1.ai_ci.begin(); i2!=f1.ai_ci.end(); i2++)
	          for (vector<pair<double, double> >::iterator j2=f2.ai_ci.begin(); j2!=f2.ai_ci.end(); j2++){
	            ...
	            Sij_t+=MurDavPrimIntegrals().Sij(f1.nx, f1.ny, f1.nz, f1.x, f1.y, f1.z, (*i2).first,
                                                   f2.nx, f2.ny, f2.nz, f2.x, f2.y, f2.z, (*j2).first)*C;
	           ...
	           for (int k=0; k<int(atoms.size()); k++)
                       Vij_t+=MurDavPrimIntegrals().Vij(f1.nx, f1.ny, f1.nz, f1.x, f1.y, f1.z, (*i2).first,
							...
							atoms[k].x, atoms[k].y, atoms[k].z)*C*atoms[k].q;
...
//	расчет двухэлектронных интегралов
	ii = jj = kk = ll = 0;
	std::cout<<"Начался расчет двуэлектронных интегралов\n";
	const int ii_end=int(basisFunctions.size());
	for (vector<vector<pair <int, SingleBasisFunction> > >::iterator i=basisFunctions.begin(); i!=basisFunctions.end(); i++){
	  std::cout<<100*(ii+1)/ii_end<<'\n';
          jj = kk = ll= 0;
	  for (vector<vector<pair <int, SingleBasisFunction> > >::iterator j=basisFunctions.begin(); j!=basisFunctions.end(); j++){
            kk = ll = 0;
...
bool InputAndMDIntegrals::set_c2v_z(int* symmAO){
	...
	for (vector< vector< pair <int, SingleBasisFunction> > >::iterator it = basisFunctions.begin(); it != basisFunctions.end();  it++){
		...
		for (vector< pair <int, SingleBasisFunction> >::iterator jt = (*it).begin(); jt != (*it).end(); jt++){
			...
			if (symm == 0) symm = symm_;
			if (symm != symm_ ) symm = -1;
		}
		if ((!isGood)||(symm<1)) {
...
int InputAndMDIntegrals::get_Z(const string& str_) const {
	...
	if ((str=="H" )||(str=="HYDROGEN" )) return 1;
	if ((str=="HE")||(str=="HELIUM"   )) return 2;
	if ((str=="LI")||(str=="LITHIUM"  )) return 3;
	if ((str=="BE")||(str=="BERYLLIUM")) return 4;
	if ((str=="B" )||(str=="BORON"    )) return 5;
	if ((str=="C" )||(str=="CARBON"   )) return 6;
	if ((str=="N" )||(str=="NITROGEN" )) return 7;
	if ((str=="O" )||(str=="OXYGEN"   )) return 8;
	if ((str=="F" )||(str=="FLUORINE" )) return 9;
	if ((str=="AR")||(str=="ARGON"    )) return 18;
...
int InputAndMDIntegrals::get_L(const string& str_) const {
	string str(str_);
	transform(str.begin(), str.end(), str.begin(), toupper);
	if ((str=="S")||(str=="L0")) return 0;
	if ((str=="P")||(str=="L1")) return 1;
	if ((str=="D")||(str=="L2")) return 2;
	if ((str=="F")||(str=="L3")) return 3;
	if ((str=="G")||(str=="L4")) return 4;
	if ((str=="H")||(str=="L5")) return 5;
	if ((str=="I")||(str=="L6")) return 6;
	if ((str=="J")||(str=="L7")) return 7;
	if ((str=="K")||(str=="L8")) return 8;
	if (str=="L9") return 9;
	if (str=="L10") return 10;
	if (str=="L11") return 11;
	return -1;
}
