...
int MurDavPrimIntegrals::calcEij3(double& Eij, double& Eij2p, double& Eij2m,
	...
	Eij2m=0;
	if (jmax>1) Eij2m=Etmp[(tmax-5)*tmax];
	delete [] Etmp;
	return 0;
}
...
double MurDavPrimIntegrals::Tij(const int& nx1, const int& ny1, const int& nz1, const double& x1, const double& y1, const double& z1, const double& alpha1,
	...
	double T_ij=-2*alpha2*alpha2*S_ij2p+alpha2*(2*nx2+1)*S_ij-0.5*nx2*(nx2-1)*S_ij2m;
	double T_kl=-2*alpha2*alpha2*S_kl2p+alpha2*(2*ny2+1)*S_kl-0.5*ny2*(ny2-1)*S_kl2m;
...
// С единичным положительным зарядом и без знака -
