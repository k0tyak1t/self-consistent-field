    ...
    for (int v = 0; v < std_m.get_nAO(); ++v) {
      ...
      for (int l = 0; l < std_m.get_nAO(); ++l) {
        for (int s = 0; s < std_m.get_nAO(); ++s) {
          eri_matrix[m][v] += density[l][s] * (std_m.get_Vee(m, v, s, l) -
                                               std_m.get_Vee(m, l, s, v) / 2);
        }
      }
    }
  }
...
void RHF::solve_rhf() {
  ...
  while (!is_converged) {
    ...
    // uncommented when diis implemented update_buffer(fock_buffer,
    // fock_matrix); update_buffer(error_buffer, error_matrix);
    common_step();
  }
