...
matrix::matrix(int n_new): n(n_new)
{
...
{
	for (int i=0; i<n; ++i){
		for (int j=0; j<n; ++j) std::cout<<std::setw(10)<<std::setprecision(6)<<_matrix_elements[i*n+j]<<' ';
		std::cout<<'\n';
	}
	return 0;
}
...
{
	if (check_ij(i,j,"get_element")>0) return 1;
	return _matrix_elements[i*n+j];
}

int matrix::get_row(const int i, double* a) const
{
	if (check_ij(i, 0, "get_row")>0) return 1;
	for(int j=0; j<n; j++) a[j]=_matrix_elements[i*n+j];
	return 0;
}
...
{
	if (check_ij(i,j,"set_element")>0) return 1;
...
{
	if (check_ij(i,0,"set_row")>0) return 1;
	for(int j=0; j<n; j++) _matrix_elements[i*n+j]=a[j];
	return 0;
}
